package com.FullyStar.mapper.clickhouse;
...
//    }
    public static void createTable(String tableSql) throws Exception{
        tableSql = "create table test(\n" +
                "key  String,\n" +
                " value Float64 ) engine =MergeTree order by (key);";
        Connection connection = getConn();
        Statement statement = connection.createStatement();
        boolean execute = statement.execute(tableSql);
        if(execute){
...
//    }
    ...
    public static void insertOne() throws Exception{
        Connection connection = getConn();
        PreparedStatement pstmt = connection.prepareStatement("insert into test values('camera', '0.5')");
        pstmt.execute();
